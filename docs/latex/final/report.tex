...
\newcommand{\dateremise}{9 avril 2025}
...
\begin{itemize}
\item \textbf{Performance des modèles} : Claude 3.5 Sonnet démontre une supériorité marquée par rapport aux modèles \detokenize{Llama3.1:8B} et \detokenize{Qwen2.5:7B}, notamment en termes de précision d'exécution (EX) et de rapidité de traitement (TRM).

\item \textbf{Différences linguistiques} : La performance varie selon la langue traitée. En français, Claude 3.5 Sonnet et \detokenize{Qwen2.5:7B} atteignent une meilleure précision d'exécution qu'en anglais, ce qui témoigne de leur robustesse multilingue.
...
\item \textbf{Précision sémantique} : \detokenize{Qwen2.5:7B} surpasse nettement \detokenize{Llama3.1:8B} dans la précision sémantique, particulièrement en anglais (66,5\,\% contre 16,7\,\%), ce qui pourrait indiquer une meilleure compréhension contextuelle du domaine alimentaire.
\end{itemize}
...
% 4. Clarification du processus d'embeddings (après tableau 4 dans section 11)
...
% 3. Ajout de la sous-section sur NetworkX (après le début de la section 11)
