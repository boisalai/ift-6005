...
class SearchDatabaseDocumentationTool(Tool):
    name = "search_db_docs"
    description = dedent(
        """\
    ...
    def search(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Ensure top_k doesn't exceed number of columns
            top_k = min(top_k, len(self.columns))
            similarities, indices = self.index.search(query_embedding, top_k)

            # Extract results from first query
            similarity_scores = similarities[0]  # similarity scores for one query
            column_indices = indices[0]          # indices of corresponding columns

            # Format results
            results = []
            for similarity, idx in zip(similarity_scores, column_indices):
            ...
            # Add debug log
            self.logger.info(f"FAISS search with query: {query}, top_k: {top_k}")
            self.logger.info(f"FAISS search returned {len(results)} columns:")
            for r in results:
    ...
    def forward(self, query: str) -> str:
        ...
        self.logger.debug(f"Searching for query in Faiss : {query}")
        ...
        try:
            ...
            self.logger.debug(f"Search results: {response}")
