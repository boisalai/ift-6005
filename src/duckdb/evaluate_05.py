...
def cleanup_old_logs(log_dir: Path, keep_last_n: int = 20) -> None:
    """
...
class QueryResult:
    """
    The QueryResult class is used to encapsulate the result of an SQL query, 
    storing its success or failure status, the results obtained, and, if 
    applicable, an error message.
    """
    success: bool
    results: List[Any]
...
class EvaluationResult:
    """
    The EvaluationResult class is used to store and organize the results of 
    evaluating a question-answer pair, including the question ID, language, 
    original question, expected answer, agent's answer, and various evaluation 
    metrics.
    """
    question_id: int
    language: str  # Evaluation language ('fr' or 'en')
    question: str  # Question in the evaluated language 
    expected_answer: str  # Expected answer in the evaluated language
    agent_answer: Dict[str, str]  # Agent response with text and source
    metrics: Dict[str, Any]  # Simplified evaluation metrics
    expected_sql: str = ""  # Expected SQL query
    agent_sql: str = ""  # SQL query generated by agent
...
class Evaluator:
    ...
    def evaluate_single_case(self, agent, qa_pair: Dict[str, Any], lang: str) -> EvaluationResult:
        ...
        try:
            ...
            # Extraire la requête SQL de l'agent (s'il y en a une)
            agent_sql = response_data.get('sql_query', '')

            # Calculer les métriques simplifiées
            sql_accuracy, execution_success = self._calculate_sql_accuracy(response_data, qa_pair)
    ...
    def _calculate_sql_accuracy(self, response_data: dict, qa_pair: Dict[str, Any]) -> Tuple[float, float]:
        """
    ...
    def _evaluate_search_sequence(self, response_data: dict) -> dict:
        """
        Evaluates the search sequence with two indicators:
        1. Sequence respect (binary: 1.0 or 0.0)
        2. Number of steps performed
        
        Returns:
            dict: {
                "sequence_respect": 1.0 or 0.0,
        ...
        try:
            ...
            db_attempts = []
            web_attempt = None
            sequence_respected = True  # By default, assume sequence is respected

            # Analyze sequence of steps
            for step in steps:
                if step['action'] in ['database_query', 'alternative_query']:
                    db_attempts.append(step)
                    # Check if DB query comes after web search (violation)
            ...
            # Binary score for sequence respect
            sequence_score = 1.0 if sequence_respected else 0.0

            # Simple count of steps
    ...
    def evaluate_all(self, agent, lang: str, max_cases: int = None) -> Dict[str, float]:
        ...
        for idx, qa_pair in enumerate(qa_pairs, start=1):
            try:
                ...
                self.logger.info(f"Evaluating question {idx}/{len(qa_pairs)}")
    ...
    def calculate_metrics(self, results: List[EvaluationResult]) -> Dict[str, float]:
        """
        Calculate global metrics from simplified evaluation results.
        
        Args:
            results: List of evaluation results
                
        Returns:
            Dict[str, float]: Calculated global metrics
        """
        if not results:
            return {
                "sql_accuracy": 0.0,
        ...
        execution_success = mean([r.metrics.get("execution_success", 0.0) for r in results])
        ...
        metrics = {
            ...
            "execution_success": execution_success * 100,
    ...
    def _log_detailed_results(self, results: List[EvaluationResult], metrics: Dict[str, float]):
        """
        Write detailed evaluation results to log file.
        First accumulate the report in a string then write it in one call.
        """
        # Initialize report string
        report_lines = []
        report_lines.append("====== EVALUATION REPORT - OPEN FOOD FACTS AGENT ======")
        report_lines.append(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Execution information
        report_lines.append("\n----- EXECUTION INFORMATION -----")
        report_lines.append(f"Language: {results[0].language}")
        report_lines.append(f"Questions processed: {len(results)}")
        report_lines.append(f"LLM Model: {self.model.model_id}")
        report_lines.append(f"Total time: {sum(r.metrics['response_time'] for r in results):.2f}s")
        report_lines.append(f"Average time per question: {metrics['avg_response_time']:.2f}s")

        # Performance metrics 
        report_lines.append("\n----- PERFORMANCE METRICS -----")
        report_lines.append(f"SQL Accuracy: {metrics['sql_accuracy']:.2f}%")
        report_lines.append(f"(+) Execution Success: {metrics['execution_success']:.2f}%")
        report_lines.append(f"Semantic Accuracy: {metrics['semantic_accuracy']:.2f}%")
        report_lines.append(f"Sequence Respect: {metrics['sequence_respect']:.2f}%")
        report_lines.append(f"Success Rate: {metrics['success_rate']:.2f}%")
        ...
        report_lines.append(f"Average: {metrics['avg_response_time']:.2f}s")
        report_lines.append(f"Median: {metrics['median_response_time']:.2f}s")
        ...
        for idx, result in enumerate(results, start=1):
            ...
            report_lines.append(f"- SQL Accuracy: {result.metrics.get('sql_accuracy', 0.0)*100:.2f}%")
            report_lines.append(f"(+) - Execution Success: {result.metrics.get('execution_success', 0.0)*100:.2f}%")
            report_lines.append(f"- Semantic Accuracy: {result.metrics.get('semantic_accuracy', 0.0)*100:.2f}%")
            report_lines.append(f"- Sequence Respect: {result.metrics.get('sequence_respect', 0.0)*100:.2f}%")
            report_lines.append(f"- Response Time: {result.metrics.get('response_time', 0.0):.2f}s")
            ...
            # Response excerpts
            report_lines.append(f"Expected answer (excerpt): {result.expected_answer[:500]}...")
            report_lines.append(f"Agent answer (excerpt): {result.agent_answer.get('text', '')[:500]}...")
            report_lines.append(f"Source: {result.agent_answer.get('source', 'Not specified')}")
            
            # Display SQL queries
            report_lines.append("Expected SQL:")
            report_lines.append(f"{result.expected_sql}")
            report_lines.append("Agent SQL:")
            report_lines.append(f"{result.agent_sql}")
            
            report_lines.append("-" * 40)  # Separator between questions
        ...
        # Write complete report in one call
...
def main():
    ...
    # Initialize model
    ENGINE = "ollama/qwen2.5:7b-instruct"
    ENGINE = "Anthropic"
    ENGINE = "ollama/llama3.1:8b-instruct-q8_0"

    if ENGINE == "ollama/llama3.1:8b-instruct-q8_0":
        model = LiteLLMModel(
            model_id="ollama/llama3.1:8b-instruct-q8_0",
            api_base="http://localhost:11434",
            num_ctx=8192
        )
    elif ENGINE == "ollama/qwen2.5:7b-instruct":
        model = LiteLLMModel(
        ...
        # See https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct
    ...
    elif ENGINE == "Qwen-14B":
        # See https://huggingface.co/Qwen/Qwen2.5-Coder-14B-Instruct
        model = TransformersModel(
            model_id="Qwen/Qwen2.5-Coder-14B-Instruct",
            device_map="auto",            # Utilise automatiquement le GPU
            torch_dtype="bfloat16",       # Format optimal pour l'A100
            trust_remote_code=True,       # Nécessaire pour Qwen
        )
    elif ENGINE == "Qwen-32B":
        # See https://huggingface.co/Qwen/Qwen2.5-Coder-32B-Instruct
        model = TransformersModel(
    ...
    elif ENGINE == "mlx/Qwen-3B":
        # See https://huggingface.co/mlx-community/Qwen2.5-Coder-3B-Instruct-8bit
        model = MLXModel(
            model_id="mlx-community/Qwen2.5-Coder-3B-Instruct-8bit"
        )
    elif ENGINE == "mlx/Qwen-14B":
        # See https://huggingface.co/mlx-community/Qwen2.5-Coder-14B-Instruct-4bit
    ...
    # Run evaluation
    lang = 'en'  # or 'fr', 'en'
    max_cases = 20  # Set to None to evaluate
    metrics = evaluator.evaluate_all(agent, lang, max_cases)
    
    # Print results
    print("\nEvaluation Results:")
    print(f"Language: {lang}")
    print(f"SQL Accuracy: {metrics['sql_accuracy']:.2f}%")
    print(f"(+) Execution Success: {metrics['execution_success']:.2f}%")
